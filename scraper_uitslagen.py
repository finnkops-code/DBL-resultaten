"""
DBL Uitslagen Scraper — Playwright (headless Chrome)

Zoekt teruglopend door afgelopen weken totdat gespeelde wedstrijden
gevonden zijn (data-state="played"). Schrijft alleen de "uitslagen"
sleutel in schedule.json, laat "programma" onaangeroerd.
...
def past_weeks():
    """Afgelopen weken, meest recent eerst."""
    today = (dt.datetime.now(timezone.utc) + timedelta(hours=2)).date()
    result = []
    for w in WEEKS:
        sunday = dt.date.fromisocalendar(w["year"], w["week"], 5) + timedelta(days=2)
        if sunday < today:
            result.append(w)
    result.reverse()
    return result
...
def main():
    print(f"DBL uitslagen scraper — {dt.datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    with sync_playwright() as p:
        ...
        for w in past_weeks()[:4]:
            ...
            print(f"  → {len(played)} gespeelde wedstrijden")
    ...
    # Lees bestaande JSON, update alleen "uitslagen"
    data = {}
    ...
    data["bijgewerkt_uitslagen"] = dt.datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    data["uitslagen_week"]       = uitslagen_week
    data["uitslagen"]            = uitslagen

    with open(JSON_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print(f"\n✅ {JSON_FILE} bijgewerkt — {len(uitslagen)} uitslagen")
